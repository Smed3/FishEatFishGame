...
namespace FishEatFish
{
    internal class ManagerNPC
    {
        ...
        private void MakeFish(int level)
        {
            ...
            if (direction == "left")
                location.X = rnd.Next(form.ClientSize.Width, form.ClientSize.Width + image.Width * 2);
            else
                location.X = rnd.Next(-2 * image.Width, -image.Width);
        ...
        public int GenerateLevel()
        {
            ...
            double mean = Player.MaxLevel;
            normalDistribution = new Normal(mean, 2);
            ...
            return roundedFishLevel < Player.MaxLevel ? rnd.Next(1, roundedFishLevel) : roundedFishLevel;
        }
